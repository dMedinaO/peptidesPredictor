...
dictElement ={"upstream":[], "NHR":[], "PFD":[], "downstream": []}#diccionario con la informacion de las secuencias y los motivos conservados
...
for record in SeqIO.parse(alignmentFile, "fasta"):
    ...
    response = searchElementInPositions(0, 26, 0, record.seq)
    if response == 0:
        dictElement["upstream"].append(record.id)

    #buscamos el motivo 2
    response = searchElementInPositions(26, 45, 0, record.seq)
    if response == 0:
        dictElement["NHR"].append(record.id)

    #buscamos el motivo 3
    response = searchElementInPositions(45, 70, 0, record.seq)
    if response == 0:
        dictElement["PFD"].append(record.id)

    #buscamos el motivo 4
    response = searchElementInPositions(70, 94, 0, record.seq)
    if response == 0:
        dictElement["downstream"].append(record.id)
